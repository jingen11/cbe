        ...
        {props.workers.workers.length > 0 ?
          (<React.Fragment>
            ...
            {props.workers.workers.map((worker, index) => {
              return <WorkerRow key={index} worker={worker} />
            })}
          </React.Fragment>) : <EmptyPage className="minus-padding" description={'No workers found...'} />}
      </div>
      <AddWorkerModal isOpen={isOpen} toggleModal={(isOpen) => toggleModal(isOpen)} />
      <FloatingActionButton onClick={() => toggleModal(!isOpen)}>
